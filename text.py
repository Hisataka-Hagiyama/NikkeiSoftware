...
button1=tk.Button(c_frame,text='<',font=('',10),command=lambda:disp(-1))
button1.grid(row=0,column=0,pady=10)
label.grid(row=0,column=1)
button2=tk.Button(c_frame,text='>',font=('',10),command=lambda:disp(1))
...
title=tk.Label(t_frame,text=make_text_1(yer[0],mon[0],today)+'のメモ',font=('',12))
title.grid(row=0,column=0,padx=20)

button = tk.Button(t_frame,text='保存',command=lambda:save(make_text_2(yer[0],mon[0],click_day)))

button.grid(row=0,column=1)
t_frame.grid(row=0,column=0,pady=10)


#メモ用のtextウィジェットとscrollウィジェット
